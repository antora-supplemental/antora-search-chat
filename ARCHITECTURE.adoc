...
*Do not* set `overflow: hidden` on `#search-field` / `.adt-search-compound`. Lunr appends `.search-result-dropdown-menu` as an absolutely positioned child (`top: 100%`); clipping the compound hides all results. Round the pill via radii/`overflow` on `.adt-search-scope-slot` and `.adt-search-input-wrap` instead.
