...
* Class hooks: `.adt-search-compound` wraps `.adt-search-scope-slot` (trigger face + `.adt-search-scope-menu` tree) + `.adt-search-input-wrap` (`#search-input` + `.adt-search-ph` overlay).
...
* Scope: conjoined dropdown on the *left edge* of the field (`.adt-search-compound` / `.adt-search-scope-slot`) — static tree *All docs* → *{component}* → *version …*; defaults per <<_default_scope_recommendation>>.
