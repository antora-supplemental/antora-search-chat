...
----
┌──────────────────────────────────────────────┐
│ Component  Valentus Theme ▾ │ Search or Ask / │
└──────────────────────────────────────────────┘
  ^kind (ink) ^current (muted)  ^input+hint
----

Closed control uses a dual-tone *face* (`.adt-search-scope-face`): primary kind label (*Component* / *Version* / *All docs*) plus muted current name/version from Antora page context (`data-component-title`, `data-version-display` on `[data-adt-search-chat]`). Native `<select>` text is transparent when closed (options stay opaque) so the menu can still say *This component — {title}*.

[.themed-svg]
image::diagrams/search-compound-layout.svg[Search compound control layout]
...
* Class hooks: `.adt-search-compound` wraps `.adt-search-scope-slot` (face + full-height `.adt-search-scope`) + `.adt-search-input-wrap` (`#search-input` + `.adt-search-ph` overlay).
...
*Collapse-to-icon*:: When leftover middle space is too narrow (high zoom / mobile), `site-search-chat.js` adds `.is-collapsed` and shows a search icon toggle. Opening it drops `.adt-search-chat__popover` under the icon with the full compound (scope + input + Ask) and lunr results. `/` and `?` open the popover. Themes should keep `.adt-navbar-search` in normal flow (not absolute-centered with a large `min-width`) so brand/title and search never paint on top of each other.

*Do not* set `overflow: hidden` on `#search-field` / `.adt-search-compound` *or* `.adt-search-input-wrap`. Lunr's `search-ui.js` mounts `.search-result-dropdown-menu` on `#search-input.parentNode` (`position:absolute; top:100%`). With compound markup that parent is `.adt-search-input-wrap`, not `#search-field` — so clipping the wrap (a common "move overflow off the compound" mistake) still hides all results. Keep both compound and input-wrap `overflow: visible`; clip typed text on `#search-input` itself (`overflow: hidden`); clip only `.adt-search-scope-slot` for the left segment chrome.

*Horizontal alignment*:: Do not force `left: 0` on the menu under the compound. That aligns only to the input cell (large empty band under the scope, tight right). This package sets `right: 0` + `width: 100cqi` against a `container-type: inline-size` compound so both edges match the full pill.
...
Themes may still fall back to a plain lunr field (`placeholder="Search"`) when `SITE_SEARCH_CHAT` is unset. The fake placeholder overlay ships in this package (`adt-search-chat` partial + `site-search-chat.css` / `.js`). Themes that keep only `#search-input` should either:
